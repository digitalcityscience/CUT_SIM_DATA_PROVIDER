...
"""
This script is supposed to be run once in order to create the noise simulation input 
from the publicly available streets_data
app_data on the geoportal. Can be re-run if street app_data is updated on portal (happens every few years :D )
Download the app_data from the geoportal and put it into the folders in the "geoportal_layers" dir.
Lots of duplications in the code. However the input app_data varies at different points slighty, that would make a 
...
def extract_residential_streets_and_motorways_to_files():
    ...
    main streets_data and district streets_data. Properties can be used to create a unique identifier.
    Substract main, district streets_data and motorways from all streets_data to get the remaining residential streets_data.
    """
    ...
    # continue with all streets_data without motorways to filter down to residential streets_data only
    ...
    for gdf in [all_streets, main_streets, district_streets]:
        gdf["unique_key"] = (
            gdf["strassenschluessel"].astype(str)
            + gdf["von_station"].astype(str)
            + gdf["bis_station"].astype(str)
        )

    all_streets["is_in_others"] = all_streets["unique_key"].isin(
        main_streets["unique_key"]
    ) | all_streets["unique_key"].isin(district_streets["unique_key"])
...
def make_noise_data_for_main_streets():
    TRAFFIC_COUNTS_JSON_STREETS = f"{os.getcwd()}/geoportal_layers/verkehrsmengen_json/de_hh_up:verkehrsmengen_dtv_hvs_2019_EPSG:25832.json"
    ...
    main_streets_gdf["max_speed"] = main_streets_gdf["geschwindigkeit"].apply(
        lambda x: extract_max_speed(x)
    )

    # add traffic values
    main_streets_gdf["truck_traffic_daily"] = round(
        main_streets_gdf["dtv"] * (main_streets_gdf["sv"] / 100)
    )
    main_streets_gdf["car_traffic_daily"] = round(
        main_streets_gdf["dtv"] - main_streets_gdf["truck_traffic_daily"]
    )
...
def make_noise_data_for_motorways():
    TRAFFIC_COUNTS_JSON_MOTORWAYS = f"{os.getcwd()}/geoportal_layers/verkehrsmengen_json/de_hh_up:verkehrsmengen_dtv_hvs_2019_EPSG:25832.json"
    traffic_counts_gdf = gpd.read_file(TRAFFIC_COUNTS_JSON_MOTORWAYS, "EPSG:25832")

    motorways = read_file_as_gdf(
        "create_app_data/geoportal_layers/strategisches_strassennetz_json/app:motorways_EPSG:25832.json",
        "EPSG:25832",
    )
    ...
    motorways["car_traffic_daily"] = round(
        motorways["dtv"] - motorways["truck_traffic_daily"]
    )
...
def make_noise_data_for_district_streets():
    district_streets_gdf = read_file_as_gdf(
        "geoportal_layers/bezirksstrassen/app:bezirksstrassen_EPSG:4326.json"
    )

    # extract first integer from strings like '30 km/h (Mo-Fr 6-19 h, sonst 50km/h)|50 km/h',
    # returns NaN for str not starting with a number
    district_streets_gdf["max_speed"] = district_streets_gdf["geschwindigkeit"].apply(
        lambda x: extract_max_speed(x)
    )

    """
    add traffic values
    # standard values for district streets_data according to UBA , p. 32, Tabelle 3.1
    ...
    district_streets_gdf["car_traffic_daily"] = round(
        300 - district_streets_gdf["truck_traffic_daily"]
    )
...
def make_noise_data_for_residential_streets():
    ...
    # extract first integer from strings like '30 km/h (Mo-Fr 6-19 h, sonst 50km/h)|50 km/h',
    # returns NaN for str not starting with a number
    residential_streets_gdf["max_speed"] = residential_streets_gdf[
        "geschwindigkeit"
    ].apply(lambda x: extract_max_speed(x))
    ...
    residential_streets_gdf["car_traffic_daily"] = round(
        100 - residential_streets_gdf["truck_traffic_daily"]
    )
...
if __name__ == "__main__":
    ...
    all_input.to_file("streets_data/streets_data.gpkg", driver="GPKG")
